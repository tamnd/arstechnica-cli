...
type App struct {
	client *arstechnica.Client
	cfg    arstechnica.Config
...
func Root() *cobra.Command {
	app := &App{cfg: arstechnica.DefaultConfig()}

	root := &cobra.Command{
		Use:   "ars",
		Short: "Read Ars Technica technology and science articles",
		Long: `ars reads Ars Technica through its public Atom RSS feeds.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

ars is an independent tool and is not affiliated with Ars Technica or Condé Nast.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	root.AddCommand(
		app.articleCmd("top", "All recent Ars Technica articles", "index", 20),
		app.articleCmd("tech", "Technology Lab articles", "technology", 20),
		app.articleCmd("science", "Science articles", "science", 20),
		app.articleCmd("gaming", "Gaming articles", "gaming", 20),
		app.articleCmd("security", "Security articles", "security", 20),
		app.articleCmd("policy", "Tech Policy articles", "policy", 20),
		app.articleCmd("business", "Business articles", "business", 20),
		app.articleCmd("cars", "Cars articles", "cars", 20),
		app.feedCmd(),
		app.sectionsCmd(),
		newVersionCmd(),
	)
...
func (a *App) setup() error {
	...
	a.client = arstechnica.NewClient(a.cfg)
	return nil
}
