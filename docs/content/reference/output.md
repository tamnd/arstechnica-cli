...
describes what users get. Pick a format with `-o`, or let ars choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ars <command> -o table   # aligned columns for reading
ars <command> -o jsonl   # one JSON object per line, for piping
ars <command> -o json    # a single JSON array
ars <command> -o csv     # spreadsheet friendly
ars <command> -o tsv     # tab-separated
ars <command> -o url     # just the URL column
ars <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ars <command> --fields id,title,url
```
...
```bash
ars <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ars <command>            # a table, because this is a terminal
ars <command> | wc -l    # JSONL, because this is a pipe
```
